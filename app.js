...
// Если Bybit не открывается без VPN, вставь сюда URL своего Cloudflare Worker.
// Пример: const bybitProxyUrl = "https://bybit-proxy.username.workers.dev";
const bybitProxyUrl = "https://bybit-proxy.ledenford.workers.dev";
const binanceTickerApi = "https://fapi.binance.com/fapi/v1/ticker/24hr";
const binanceMarkApi = "https://fapi.binance.com/fapi/v1/premiumIndex";
const binanceKlineApi = "https://fapi.binance.com/fapi/v1/klines";
const binanceExchangeInfoApi = "https://fapi.binance.com/fapi/v1/exchangeInfo";
const usdRubApi =
...
function getBinanceTickerUrl(symbol) {
  return `${binanceTickerApi}?symbol=${symbol}`;
}

function getBinanceMarkUrl(symbol) {
  return `${binanceMarkApi}?symbol=${symbol}`;
}

function getBinanceKlineUrl(symbol) {
  return `${binanceKlineApi}?symbol=${symbol}&interval=15m&limit=${chartLimit}`;
}
...
async function fetchBybitJson(path) {
  let lastError;
  const endpoints = bybitProxyUrl
    ? [bybitProxyUrl.replace(/\/$/, ""), ...bybitHosts]
    : bybitHosts;
...
async function fetchRates() {
  ...
  try {
    ...
    const [tickerData, usdRub] = await Promise.all([
      Promise.all(futuresAssets.map((asset) => fetchFuturesTicker(asset.symbol))),
      fetchUsdRubRate(),
    ]);
    ...
    statusPill.textContent = rows.some((row) => row.source === "Binance")
      ? "Онлайн · резерв"
      : "Онлайн";
  } catch (error) {
...
async function fetchFuturesTicker(symbol) {
  try {
...
async function fetchBinanceTicker(symbol) {
  const [ticker, mark] = await Promise.all([
    fetchJson(getBinanceTickerUrl(symbol)),
    fetchJson(getBinanceMarkUrl(symbol)),
  ]);
...
async function loadBybitInstruments() {
  try {
    ...
    await loadBinanceInstruments();
  }
}

async function loadBinanceInstruments() {
  try {
    const data = await fetchJson(binanceExchangeInfoApi);
    ...
    instruments = (data.symbols || [])
      ...
      .map((instrument) => ({
        symbol: instrument.symbol,
        baseCoin: instrument.baseAsset || instrument.symbol.replace(/USDT$/, ""),
      }))
      .sort(sortInstruments);

    populateSymbolSelect();
    addStatus.textContent = "Bybit не открылся, список взят из Binance Futures";
  } catch (error) {
    instruments = [...fallbackInstruments];
    populateSymbolSelect();
    addStatus.textContent = "Список биржи не загрузился, показаны популярные пары";
  }
}
...
async function fetchChartData(symbol) {
  ...
  try {
    candles = await fetchBybitChartData(symbol);
  } catch (error) {
    candles = await fetchBinanceChartData(symbol);
  }
...
async function fetchBinanceChartData(symbol) {
  const data = await fetchJson(getBinanceKlineUrl(symbol));
